...
We will tell the LUMI login node to forward requests from Docker's IP address on local port 8042 to port 8042 on the compute node's IP address. There are 3 'hops': Container -> Host -> LUMI Login -> LUMI Compute. 
...
```bash
# Replace <NODELIST> and <your username>
ssh -N -L 8042:<NODELIST>:8042 <your username>@lumi.csc.fi
```
...
- `172.17.0.1` is the IP address of Docker containers on Linux.
...
To stop the agent:
```bash
docker stop opencode-sandbox && docker rm opencode-sandbox
```
To stop the tunnel:
